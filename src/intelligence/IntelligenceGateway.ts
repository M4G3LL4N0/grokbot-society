...
export interface ExternalResultReceipt {
  booked: boolean;
  cost: number;
...
export class IntelligenceGateway {
  ...
  acceptExternalResult(input: ExternalResultInput): ExternalResultReceipt {
    if (!input.providerCallRecorded) {
      return { booked: false, cost: 0, telemetryId: null };
    }
    if (!Number.isFinite(input.estimatedCost) || input.estimatedCost < 0) {
      throw new Error("external result cost must be a finite non-negative number");
    }
    const reasonKind = input.reasonKind ?? "foreground";
    ...
    try {
      const telemetryId = this.telemetry.record(
        {
          eventId: input.eventId,
          reason: input.reason,
          caller: input.caller,
      ...
      return { booked: true, cost: input.estimatedCost, telemetryId };
    } finally {
      allowance.release();
    }
  }
